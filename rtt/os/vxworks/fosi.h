...
/* Even though we want to claim to be POSIX, attempting to do so
   breaks a LOT of VxWorks headers. */
...
/* VxWorks defines FD_ZERO (in <sys/select.h>) as a macro around
   bzero(), but fails to include the required <strings.h> to make
   bzero() available.  Rather than add #ifdef __VXWORKS__ all over the
   place, we include it here. */
#include <strings.h>
